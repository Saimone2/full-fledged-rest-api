...
import com.saimone.full_fledged_rest_api.model.*;
import com.saimone.full_fledged_rest_api.repository.ConfirmTokenRepository;
...
import com.saimone.full_fledged_rest_api.service.AuthenticationService;
import com.saimone.full_fledged_rest_api.service.MailSenderService;
...
public class AuthenticationServiceImpl implements AuthenticationService {
    ...
    protected void sendResetPasswordToEmail(User user) {
        String confirmationToken = UUID.randomUUID().toString();
    ...
    public ResponseEntity<Object> confirmEmail(String token) {
        ...
        if (optionalToken.isPresent()) {
            ...
            if (!tokenModel.expired && !tokenModel.revoked && tokenModel.getTokenAssignment() == TokenAssignment.EMAIL_CONFIRMATION) {
                ...
                tokenModel.setUpdatedAt(Instant.now());
        ...
        if (optionalUser.isPresent()) {
            ...
            if (!user.isEnabled()) {
                ...
                if (existingToken.size() > 1) {
                    ...
                    throw new MessageAlreadySentException("Please check your email inbox as the message has already been sent to you.");
                }
                ...
                return ResponseHandler.responseBuilder("A confirmation email has already been sent to this email address. Please check your inbox. If the message has not arrived, try resending it in " + confirmTokenLifetime + " minutes.", HttpStatus.OK, response);
            } else {
    ...
    public ResponseEntity<Object> changePassword(ResetPasswordRequest request) {
        ...
        if (optionalToken.isPresent()) {
            ...
            if (!confirmToken.isExpired() && !confirmToken.isRevoked() && confirmToken.getTokenAssignment() == TokenAssignment.RESET_PASSWORD) {
                ...
                confirmToken.setUpdatedAt(Instant.now());
